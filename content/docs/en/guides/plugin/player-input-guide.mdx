...
```java
// 1. Grant permission on join
player.addPermission("hytale.editor.builderTools");

// 2. Intercept in packet handler
void handleBuilderToolAction(BuilderToolGeneralAction packet, PlayerRef player) {
    if (player.getGameMode() != GameMode.Creative) {
        // Hijack for abilities
        switch (packet.action) {
            case ActivateToolMode -> castFireball(player);    // B pressed
            case DeactivateToolMode -> endFireball(player);   // B released
            case HistoryUndo -> castFireTrail(player);        // Ctrl+Z
            case HistoryRedo -> castHaste(player);            // Ctrl+Y
            case SelectionCopy -> castVanish(player);         // Ctrl+C
        }
        return;  // Don't run actual builder tool logic
    }
    // Creative players get normal behavior
    super.handleBuilderToolAction(packet, player);
}
```
